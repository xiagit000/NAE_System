	...
	public boolean preHandle(HttpServletRequest request,
		...
		if(handler instanceof HandlerMethod){
			...
			if(null==requires){
				...
				Object Exsituser=session.getAttribute("currentUser");
				if(null==Exsituser){
					response.sendRedirect(request.getContextPath());
					return false;
				}
				User user = (User)Exsituser;

				Integer notificationCount = receiverNotificationDao.countNoConsultNotificationByUser(user);
