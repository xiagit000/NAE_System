...
public class LearnClassServiceImpl implements LearnClassService {
	...
	public LearnClass findByTeacher(User user) {
		return this.learnClassDao.findByTeacher(user);
	}

	@Override
	public List<LearnClass> listByLearnSubProjectInCurrentBatch(
			LearnSubProject learnSubProject, Batch currentBatch) {
		return learnClassDao.listByLearnSubProjectInCurrentBatch(learnSubProject,currentBatch);
	}

	@Override
	public List<LearnClass> listByLearnAreasInCurrentBatch(
			List<LearnArea> learnAreas, Batch currentBatch) {
		List<LearnClass> result  = Lists.newArrayList();
		for(LearnArea learnArea : learnAreas){
			List<LearnClass> lcs = learnClassDao.listByLearnAreaInCurrentBatch(learnArea,currentBatch);
			for(int i =0;i<lcs.size();i++){
				result.add(lcs.get(i));
			}
		}
		return result;
	}
	...
	@Override
	public List<LearnClass> listByLearnAreaInCurrentBatch(LearnArea temp,
			Batch currentBatch) {
		return learnClassDao.listByLearnAreaInCurrentBatch(temp, currentBatch);
	}
	...
	public List<LearnClass> listWithPagination(Integer page) {
		return learnClassDao.listWithPagination(page);
	}

	@Override
	public Integer getClassCount() {
		return learnClassDao.getClassCount();
	}
