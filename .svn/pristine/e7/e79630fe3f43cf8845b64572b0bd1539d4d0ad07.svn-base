...
public class SchoolRollDaoImpl extends BaseDaoImpl<SchoolRoll, Long> implements SchoolRollDao {
	...
	public boolean checkExistByUser(User user) {
		String queryString="from SchoolRoll sr where sr.user=?1";
	...
	public List<SchoolRoll> listActiveByClassWithPagination(LearnClass learnClass,
			Integer page) {
		String queryString = "FROM SchoolRoll sr WHERE sr.learnClass=?1 AND sr.newest=?2";
		return executeQueryWithPagination(queryString, DEFAULT_ORDER, page, learnClass,true);
	}

    @Override
    public List<SchoolRoll> findSchoolRollsByDiscipline(Discipline discipline) {
        String queryString = "FROM SchoolRoll sr WHERE sr.learnClass.learnProject.discipline=?1";
