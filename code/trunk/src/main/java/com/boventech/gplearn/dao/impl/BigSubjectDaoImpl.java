...
public class BigSubjectDaoImpl extends BaseDaoImpl<BigSubject,Long> implements
	...
	public List<BigSubject> listWithDisciplineInCurrentBatch(
			Discipline discipline, Batch currentBatch) {
		String queryString = "FROM BigSubject bsj WHERE bsj.batch=?1 AND discipline=?2";
		return executeQueryWithoutPaging(queryString, currentBatch,discipline);
	}
