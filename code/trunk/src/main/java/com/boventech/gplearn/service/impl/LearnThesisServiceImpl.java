...
public class LearnThesisServiceImpl implements LearnThesisService {
	...
	@Override
	public List<LearnThesis> lisOverThesisWithBigSubjects(
			List<BigSubject> bigsubjects) {
		Date date =new Date();
		List<LearnThesis> result = Lists.newArrayList();
		for(BigSubject bigsubject : bigsubjects){
			List<LearnThesis> list=learnThesisDao.listOverThesisByBigSubject(bigsubject, date);
			for(int i =0;i<list.size();i++){
				result.add(list.get(i));
			}
		}
	...
	@Override
	public List<LearnThesis> listThesisWithBigSubjects(
			List<BigSubject> bigSubjects) {
		List<LearnThesis> result = Lists.newArrayList();
		for(BigSubject bigsubject : bigSubjects){
			List<LearnThesis> list = learnThesisDao.listByBigSubjet(bigsubject);
			for(int i =0;i<list.size();i++){
				result.add(list.get(i));
			}
		}
